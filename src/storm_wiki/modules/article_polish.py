...
class PolishAndRewritePage(dspy.Signature):
    """You are a skilled editor that excels at removing duplicate information and rewriting articles to ensure a cohesive and smooth flow. You will maintain the article structure (indicated by "#", "##", etc.) and inline citations while transforming the text into markdown. Your goal is to create a seamless, engaging, and casually toned article with a captivating title."""
...
class PolishPageModule(dspy.Module):
    def __init__(
        ...
        self.rewrite_page = dspy.Predict(PolishAndRewritePage)
    ...
    def forward(self, topic: str, draft_page: str, polish_whole_page: bool = True):
        ...
        if polish_whole_page:
            with dspy.settings.context(lm=self.polish_engine):
                ...
                page = self.rewrite_page(draft_page=polished_page).page
        else:
            page = draft_page
