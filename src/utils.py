...
def load_api_key(toml_file_path):
    # Load .env values first to ensure they're available as a fallback
    ...
    try:
        with open(toml_file_path, "r") as file:
            data = toml.load(file)
        # Check each key-value pair in the TOML data
        for key, value in data.items():
            # If the value is empty, fallback to the environment variable (if it exists)
            # If the value is empty, and the key exists in the environment, it will automatically use the existing value.
            # If the value is not empty, set it as the environment variable.
            os.environ[key] = str(value) if value != "" else os.environ.get(key, "")
    except (FileNotFoundError, toml.TomlDecodeError) as e:
        print(f"Error with TOML file: {toml_file_path}. {str(e)}", file=sys.stderr)
